...
function TablaDias({ datos, onDiaClick }) {
  ...
  const cols = [
    ...
    { key: 'cabezas',    label: 'Cabezas',      w: '72px', num: true },
    { key: 'cxc',        label: 'Ctos/cab',     w: '72px', num: true },
  ]
  ...
  const filas = useMemo(() => {
    return Object.values(datos).map(d => {
      ...
      return {
        ...
        cabezas: d.cabezas || null,
        cxc: d.cabezas && d.total ? Math.round(d.total / d.cabezas * 10) / 10 : null,
      }
    }).sort((a, b) => {
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
          ...
          {filas.map((f, i) => {
            ...
            return (
              <tr key={f.fecha}
                ...
                <td style={{ padding: '8px 10px', textAlign: 'right', color: f.cabezas ? '#6B48FF' : '#ddd', fontWeight: f.cabezas ? '700' : '400', fontSize: '11px' }} onClick={() => onDiaClick(f.fecha)}>{f.cabezas ? formatNum(f.cabezas) : '—'}</td>
                <td style={{ padding: '8px 10px', textAlign: 'right', color: f.cxc ? '#6B48FF' : '#ddd', fontWeight: f.cxc ? '600' : '400', fontSize: '11px' }} onClick={() => onDiaClick(f.fecha)}>{f.cxc ? f.cxc.toFixed(1) : '—'}</td>
              </tr>
            )
          })}
        </tbody>
      ...
      {seleccionados.size > 0 && (() => {
        ...
        const tCabezas = sel.reduce((s,f) => s + (f.cabezas||0), 0)
        const tCxC     = tCabezas > 0 && tT > 0 ? (tT / tCabezas).toFixed(1) : null
          ...
          <div style={{ border: `2px solid ${C.azul}`, borderTop: 'none', borderRadius: '0 0 14px 14px', background: C.azulClaro, padding: '14px 16px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '8px' }}>
              {[
                ...
                tCabezas > 0 ? ['Cabezas', formatNum(tCabezas), null, '#6B48FF', '#F5F0FF'] : null,
                tCxC ? ['Ctos/cab', tCxC, 'rendimiento', '#6B48FF', '#F5F0FF'] : null,
...
export default function Reportes({ onVolver }) {
  ...
  const [usarEjemplos, setUsarEjemplos] = useState(false)
